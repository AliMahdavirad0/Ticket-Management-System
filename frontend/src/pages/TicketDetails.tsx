...
export default function TicketDetails() {
  ...
  // Handle invalid ID
  if (!id || isNaN(ticketId) || ticketId <= 0) {
    return (
      <div className="p-6 text-center">
        <p className="text-lg text-gray-500">Invalid ticket ID.</p>
        <button onClick={() => navigate('/tickets')} className="text-sm text-blue-600 hover:underline mt-2">
          ← Back to Tickets
        </button>
      </div>
    );
  ...
  const handleStatusChange = (status: string) => {
    changeStatus.mutate({ id: ticketId, status: status as 'OPEN' | 'IN_PROGRESS' | 'RESOLVED' | 'CLOSED' });
  };

  const handlePriorityChange = (priority: string) => {
    changePriority.mutate({ id: ticketId, priority: priority as 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL' });
  };
