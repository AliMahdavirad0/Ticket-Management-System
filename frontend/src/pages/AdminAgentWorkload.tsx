...
export default function AdminAgentWorkload() {
  ...
  const totalAssigned = workloads?.reduce((sum, w) => sum + w.assigned_total, 0) ?? 0;
              ...
              <tbody>
                {workloads.map((w) => (
                  <tr key={w.id} className="border-b last:border-0 hover:bg-gray-50">
                    ...
                    <td className="py-3 text-gray-600">{w.email}</td>
                    <td className="py-3 text-center">
                      <Badge variant="info">{w.open}</Badge>
                    </td>
                    <td className="py-3 text-center">
                      <Badge variant="warning">{w.in_progress}</Badge>
                    </td>
                    ...
                    <td className="py-3 text-center font-bold">{w.assigned_total}</td>
                  </tr>
                ))}
              </tbody>
            </table>
