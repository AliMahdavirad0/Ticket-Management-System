 ...
 */
export const getUsers = async (params?: { page?: number; search?: string; role?: string }): Promise<PaginatedResponse<UserAdmin>> => {
  const response = await axiosClient.get('/accounts/users/', { params })
  return response.data
}
...
export const updateUserRole = async (id: number, role: string): Promise<UserAdmin> => {
  const response = await axiosClient.patch(`/accounts/users/${id}/role/`, { role })
  // Backend returns { message, user: UserMinimal }
